...
public class ApiService {
    ...
    public IngecResponse infoAnticipation(InfoAnticipationRequest rRequest, HttpServletRequest request) {
        log.info("Enter in infoAnticipation=== [{}]", rRequest);
        Locale locale = defineLocale(rRequest.getLangue());
        ...
        Optional<ParamEndPoint> endPoint = endPointService.findByCodeParam("infoAnticipation");
        if (!endPoint.isPresent()) {
            genericResp.setCode(ICodeDescResponse.ECHEC_CODE);
            genericResp.setDescription(messageSource.getMessage("service.absent", null, locale));
            genericResp.setDateResponse(Instant.now());
        ...
        try {
            String jsonStr = new JSONObject()
                .put("pays", rRequest.getCountry())
                .put("operation", rRequest.getOperation())
                .put("date_anticipation", rRequest.getDateAnticipation())
                .put("iddossier", rRequest.getIdDossier())
                .put("montant_restantdu", rRequest.getMontantRestantDu())
                .put("nooper", rRequest.getNooper())
                .put("ncg", rRequest.getNcg())
                .put("taxe_interet", rRequest.getTaxeInteret())
                .put("taux_penalite", rRequest.getTauxPenalite())
                .put("mensualite", rRequest.getMensualite())
                .put("created_by", rRequest.getCreatedBy())
                .put("statut", rRequest.getStatut())
                .toString();
            log.info("request infoAnticipation  [{}]", jsonStr);
            HttpURLConnection conn = utils.doConnexion(endPoint.get().getEndPoints(), jsonStr, "application/json", null, null);
            BufferedReader br = null;
            JSONObject obj = new JSONObject();
            String result = "";
            log.info("resp code infoAnticipation [{}]", conn.getResponseCode());
            ObjectMapper mapper = new ObjectMapper();
            Map<String, Object> map = mapper.readValue(obj.toString(), Map.class);
            if (conn != null && conn.getResponseCode() == 200) {
                br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
                ...
                log.info("infoAnticipation result ===== [{}]", result);
                obj = new JSONObject(result);
                ...
                log.info("resp infoAnticipation error ===== [{}]", result);
                obj = new JSONObject(result);
                genericResp.setCode(ICodeDescResponse.ECHEC_CODE);
                genericResp.setDateResponse(Instant.now());
                /*String ret = getMsgEchecRembousementTAE(obj, locale);
